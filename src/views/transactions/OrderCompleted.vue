...
<template>
    <div class="uk-container uk-width-3-4">
        ...
        v-for="item in invoiceStore.invoicesUser" v-show="item.status === 'delivered'">
        <div class="uk-margin-small-left uk-margin-small-right">
            ...
            <div class="uk-grid uk-grid-small uk-margin-small-top">
                <div class="uk-width-expand">
                    ...
                    @click="invoiceStore.callback()" style="border-color: #0CC701;font-size:17px;color: #0CC701;font-weight: 600;padding: 0px 20px;">Hubungi</button>
                </div>
        ...
        <div class="uk-margin-small-left uk-margin-small-right">
            <div class="uk-grid uk-grid-small uk-text-small uk-padding-remove" uk-grid>
                <div class="uk-width-expand">
                    <div class="uk-inline uk-margin-right uk-margin-small-left sm-txt-sk">No Invoice : </div>
                    <div class="uk-inline md-txt-sk">{{ item.id }}</div>
                </div>                
                <div class=" uk-margin-small-right  sm-txt-sk">
                    {{ item.status }}
                </div>
            </div>
            <hr class="uk-margin-auto-top uk-margin-small-bottom" style="border-top: 1px solid;border-color: green;">
            <div class="content uk-grid-small uk-card-body uk-margin-remove-top" v-for="detail in item.details" uk-grid>
                ...
                <div class="uk-width-expand">
                    <p class="uk-margin-remove md-txt-sk">{{ detail.product_id.name }}</p>
                    <p class="uk-margin-remove uk-text-muted uk-text-small">{{ detail.qty }}Barang</p>
                </div>
                <div class="uk-width-auto uk-flex uk-flex-middle uk-text-bold uk-margin-right md-txt-sk" style="color:black">
                    Rp{{ tools.rupiahNum(detail.unit_price) }}
                </div>
                <hr class="uk-width-1-1 uk-margin-auto-bottom uk-margin-small-top uk-margin-left" style="border-top: 1px solid;border-color: green;">
            </div>
            <div class="uk-grid uk-grid-small uk-margin-small-top">
                <div class="uk-width-expand">
                    <button class=" uk-button uk-button-default uk-margin-small-left uk-margin-small-bottom uk-border-rounded uk-text-capitalize md-btn-sk" 
                    @click="invoiceStore.callback()" style="border-color: #0CC701;font-size:17px;color: #0CC701;font-weight: 600;padding: 0px 20px;">Hubungi</button>
                </div>
                <div class="uk-width-auto uk-flex uk-flex-middle">
                    <div class="uk-inline uk-margin-medium-right md-txt-sk">Total Pesanan</div>
                    <div class="uk-inline uk-text-bold md-txt-sk" style="color:black">Rp{{ tools.rupiahNum(item.total_amount) }}</div>
                </div>
            </div>
            </div>
        </div>
